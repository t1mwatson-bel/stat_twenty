...
def check_and_predict():
    global all_messages, game_history
    ...
    if message_id:
        ...
        save_history(history)
        print(f"✅ ПРОГНОЗ ОТПРАВЛЕН: #{target_num} → {', '.join(cards_list)} ({method})", flush=True)
