...
async def get_table_url(page, target_game_number):
    """Получение URL стола по номеру игры с учетом реальных номеров на сайте"""
    try:
                    ...
                    if match:
                        ...
                        if link_element:
                            ...
                            if href:
                                ...
                                table_urls[current_number] = href
            except Exception as e:
                continue
        
        logging.info(f"📋 Доступные номера столов: {sorted(available_numbers)}")
        
        # Ищем точное совпадение
        if target_game_number in table_urls:
            logging.info(f"✅ Найден точный стол #{target_game_number}")
            return table_urls[target_game_number]
        
        # Если точного нет, ищем ближайший
        if available_numbers:
            # Находим ближайший номер к target_game_number
            closest = min(available_numbers, key=lambda x: abs(x - target_game_number))
            logging.info(f"⚠️ Точный номер {target_game_number} не найден, использую ближайший #{closest}")
            return table_urls.get(closest)
...
async def poll_game_short(table_url, game_number, game_start_time):
    """Короткие сессии по 2 секунды для конкретного стола"""
    ...
    # Получаем ID сообщения если уже было
    with lock:
        if game_number in message_ids:
            msg_id = message_ids[game_number]
    
    logging.info(f"🎮 Игра #{game_number}: НАЧАЛО ОПРОСА (макс. 120 сек)")
    ...
    for attempt in range(MAX_POLL_ATTEMPTS):
        ...
        browser = None
        try:
            # Открываем браузер на 2-3 секунды
            async with async_playwright() as p:
                ...
                await page.wait_for_timeout(1000)  # 1 секунда на загрузку
                ...
                if game_started and state != last_state:
                    if not first_message_sent:
                        msg_text = format_message(game_number, state, turn=turn, is_final=False)
                        sent = send_telegram_message_with_retry(CHANNEL_ID, msg_text)
                        msg_id = sent.message_id
                        first_message_sent = True
                        # Сохраняем ID сообщения
                        with lock:
                            message_ids[game_number] = msg_id
                        logging.info(f"Игра #{game_number}: ПЕРВОЕ СООБЩЕНИЕ ОТПРАВЛЕНО")
                    else:
                        msg_text = format_message(game_number, state, turn=turn, is_final=False)
                        edit_telegram_message_with_retry(CHANNEL_ID, msg_id, msg_text)
                ...
                if is_finished and game_started:
                    ...
                    final_msg = format_message(game_number, state, is_final=True)
...
def run_async_poll(table_url, game_number, game_start_time):
    """Запуск асинхронного опроса в потоке"""
    try:
        asyncio.run(poll_game_short(table_url, game_number, game_start_time))
    except Exception as e:
        logging.error(f"Критическая ошибка в игре #{game_number}: {e}")

def launch_next_game():
    """Запускает новый поток для следующей игры"""
    ...
    try:
        ...
        logging.info(f"🚀 Игра #{game_number}: ЗАПУСК ПОТОКА (старт в {game_time.strftime('%H:%M:%S')})")
        
        # СОЗДАЕМ НОВЫЙ ПОТОК для этой игры
        thread = threading.Thread(
            target=run_async_poll, 
        ...
        with lock:
            active_games[game_number] = thread
...
def clean_finished_games():
    ...
    with lock:
        finished = [g for g, t in active_games.items() if not t.is_alive()]
        for g in finished:
            del active_games[g]
            if g in message_ids:
                del message_ids[g]
...
def monitor_loop():
    ...
    global bot_running
    logging.info("🚀 Бот 21 Classic (короткие сессии) ЗАПУЩЕН")
    ...
    while bot_running:
        try:
            ...
            # Запускаем новые игры в нужное время
            if now.second >= 58 and now.minute % 2 == 1:
                next_game_time, seconds_to_next = get_next_game_time()
                current_time = time.time()
                
                if seconds_to_next <= 2 and (current_time - last_launch_time) > 58:
                    game_number = get_game_number_by_time(next_game_time)
