...
MAX_RECORDS = 50000  # Увеличил для лучшего обучения
CHECK_INTERVAL = 5
OFFSET = 1
MIN_TRAIN_SAMPLES = 100  # Уменьшил для раннего старта
MAX_HISTORY = 2000
MAX_GAME_HISTORY = 10
DOGON_GAMES = 4
ML_CONFIDENCE_THRESHOLD = 0.70
...
def train_ml_model():
    ...
    for game in data:
        ...
        # Ищем ВСЕ целевые карты в этой игре
        for card in all_cards:
            ...
            if card_str in TARGET_CARDS:
                ...
                break  # Берём первую целевую карту
    ...
    for entry in predictions:
        ...
        for i in range(max_games_to_check):
            ...
            if i == max_games_to_check - 1 and not found:
                ...
                # Ищем, какая целевая карта ВЫПАЛА на самом деле
                actual_target = None
                ...
                if actual_target:
                    ...
                    try:
                        ...
                        if features and ml_initialized:
                            ...
                            # Пытаемся сделать частичное обучение
                    ...
                    # Если вообще не выпало целевых карт
                    stats["total"] += 1
                    stats["lose"] += 1
                    stats["ml_losses"] += 1
...
def main():
    ...
    while True:
        try:
            ...
            # 🔥 ПЕРЕОБУЧЕНИЕ КАЖДЫЕ 3 МИНУТЫ НА ВСЕХ НОВЫХ ДАННЫХ
...
if __name__ == "__main__":
    ...
    if ml_cards and confidence and confidence >= ML_CONFIDENCE_THRESHOLD:
        ...
        print(f"⏭️ ML не уверен: {confidence*100:.1f}% < {ML_CONFIDENCE_THRESHOLD*100:.0f}% — пропускаем", flush=True)
        return None, None, None

# =====================================================================
# ПРОВЕРКА РЕЗУЛЬТАТА (ИДЕАЛЬНАЯ, КАК В ГИБРИДЕ)
# =====================================================================
def check_results():
